...
public class Algorithms {
	...
	{
		/*for (int i = 0; i < mas.length; i++) // Число проходов
		{
			for (int j = mas.length - 1; j > i; j--) // Внутренний цикл прохода
			{
				if (mas[j - 1] > mas[j])
				{
					int temp = mas[j - 1];
	...
	public static void quickSort(int[] mas, int low, int high)
	{
		int i = low;
		int j = high;
		int x = mas[(low + high) / 2]; // Центральный элемент
		
		// Процедура разделения относительно опорного элемента
		do {
			while (mas[i] < x)
			{
				i++;
	...
	public static void mergeSort(int[] mas, int lb, int ub)
	{
		...
		if (lb < ub)
		{
			split = (lb + ub) / 2; // Делим массив пополам
			
			// Сортируем обе половины
			mergeSort(mas, lb, split);
			mergeSort(mas, split + 1, ub);
			merge(mas, lb, split, ub); // Сливаем результат в один массив
		}
	}
	
	public static void merge(int[] mas, int lb, int split, int ub)
	{
		int pos1 = lb; // Первая последовательность
		int pos2 = split + 1; // Вторая последовательность
		int pos3 = 0;
		...
		// Копировать остаток последовательностей
		while (pos1 <= split)
		{
			temp[pos3] = mas[pos1];
			pos1++;
		...
		// Скопировать буфер в исходный массив
